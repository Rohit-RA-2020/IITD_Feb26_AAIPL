"""
A-AGENT Fine-Tuning Script (Optimized)
=====================================
...
class Config:
    ...
    # Data
    TRAIN_DATA_PATH = "data/final/answer_agent_train.json"
    ...
    # Output paths
    OUTPUT_DIR = "outputs/answer_agent_optimizedv2"
    LORA_SAVE_PATH = "models/answer_agent_lora_optimizedv2"
    MERGED_SAVE_PATH = "models/answer_agent_merged_optimizedv2"
...
def train_model(trainer, model):
    ...
    print_header("🚀 STARTING TRAINING", "=", ColoredOutput.BOLD)
...
def print_final_summary(dataset, trainer_stats, metrics_logger, training_success):
    ...
    print_header("📊 TRAINING SUMMARY", "=", ColoredOutput.OKCYAN)
...
def main():
    ...
    try:
        # Header
        print_header("🤖 A-AGENT FINE-TUNING (OPTIMIZED)", "=")
        print(f"  Model: Qwen2.5-14B-Instruct")
        print(f"  Task: Answer Agent Training")
        ...
        # Final message
        print_header("✨ TRAINING PIPELINE COMPLETE", "=", ColoredOutput.OKGREEN)
        
        if training_success:
            print(f"  {ColoredOutput.BOLD}Next Steps:{ColoredOutput.ENDC}")
            print(f"    1. Test your model with inference script")
            print(f"    2. Evaluate on held-out test set")
            print(f"    3. Compare with baseline model")
            print(f"    4. Deploy or continue training Q-Agent")
